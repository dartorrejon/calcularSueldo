//Funcion que devuelve la cantidad de dias del mes ingresado
...
//Modificamos el calendario con el evento submit del form
document.querySelector("#cambiar").addEventListener("submit", ev =>{
  ev.preventDefault();
  ev.stopPropagation();

  const $inputFecha = document.querySelector("#fechita");
  let fechita = $inputFecha.value;
  const datos = fechita.split('-');
  fechita = datos.join('/');
   
  const $titulo = document.querySelector('h2');
  $titulo.innerText = '';
  
  cargarCalencario(new Date(fechita));
})
